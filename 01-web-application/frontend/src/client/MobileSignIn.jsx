...
const MobileSignInContainer = styled.div`
  min-height: 100vh;
  width: 100vw;
  background: #ffffff;
  display: flex;
...
const StyledSignInForm = styled.div`
  ...
  .form label > *:not(input):not(.password-wrapper) {
    display: block !important;
  }
  ...
  .form label .password-wrapper input {
    color: #000 !important;
  }
  ...
  /* Show Forgot Password link in mobile mode */
...
const MobileSignIn = () => {
  ...
  const handleSignInSuccess = (userData) => {
    console.log('MobileSignIn: Sign in successful, userData:', userData);
    navigate('/');
  };
  ...
  // Override Forgot Password link to navigate to /forgotpassword page in mobile
  React.useEffect(() => {
    ...
    const setupForgotPasswordLinks = () => {
      ...
      forgotPasswordLinks.forEach(link => {
        ...
        link.addEventListener('click', handleForgotPasswordClick, true);
      });
    };

    // Run immediately and after delays to catch dynamically rendered elements
    setupForgotPasswordLinks();
    const timer1 = setTimeout(setupForgotPasswordLinks, 100);
    const timer2 = setTimeout(setupForgotPasswordLinks, 300);
    const timer3 = setTimeout(setupForgotPasswordLinks, 500);

    // Also use MutationObserver to catch dynamically added elements
    const observer = new MutationObserver(setupForgotPasswordLinks);
    const container = document.querySelector('.MobileSignInContainer');
    if (container) {
    ...
    return () => {
      ...
      const forgotPasswordLinks = document.querySelectorAll('.forgot-password-link');
      forgotPasswordLinks.forEach(link => {
        link.removeEventListener('click', handleForgotPasswordClick, true);
      });
    };
  }, [navigate]);
