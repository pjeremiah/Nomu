...
app.post('/api/user/:id/claim-reward', async (req, res) => {
  try {
    ...
    // Calculate current cycle and points for tracking
    const currentCycle = Math.floor((user.points - 1) / 5) + 1;
    ...
    if (type === 'coffee') {
      ...
      console.log(`Reset points to 0 for coffee claim`);
    } else if (type === 'donut') {
...
app.post('/api/loyalty/scan', async (req, res) => {
  try {
    const { qrToken, itemName, itemType, category, price } = req.body;
