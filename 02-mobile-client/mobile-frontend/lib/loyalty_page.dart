...
class _LoyaltyPageState extends State<LoyaltyPage> with TickerProviderStateMixin {
  ...
  void _initializeSocket() async {
    try {
      ...
      // Set up listener anyway - it will work when connection is established
      _setupLoyaltySocketListener();
    }
  }
  ...
  void _checkRewardClaimStatus() {
    if (points == null || activeRewards.isEmpty) return;
    ...
    for (final reward in activeRewards) {
      ...
      if (points! < pointsRequired) {
        rewardClaimedStatus[rewardId] = true; // Not enough points - hide reward
        LoggingService.instance.loyalty('Not enough points for $title - need $pointsRequired, have $points');
        continue;
      }
      ...
      if (pointsRequired == 5) {
        // Calculate current cycle based on points
        // This must match the backend calculation: Math.floor((points - 1) / 5) + 1
        // If points = 0, cycle = 0
        // If points = 1-5, cycle = 1
        // If points = 6-10, cycle = 2, etc.
        final currentCycle = ((points! - 1) / 5).floor() + 1;
        LoggingService.instance.loyalty('5-point reward $title - current cycle: $currentCycle, points: $points');
        
        // Check if claimed in current cycle
        for (final claim in rewardsHistory) {
          ...
          bool isMatch = false;
          if (claimType.toLowerCase().contains('donut') || 
              claimDescription.toLowerCase().contains(title.toLowerCase())) {
            isMatch = true;
          }
          
          if (isMatch) {
            // Hide if claimed in current cycle or later
        ...
        if (sessionClaimedRewards.containsKey(rewardId)) {
          ...
          for (final claim in rewardsHistory) {
            ...
            // Check if this claim matches the current reward
            bool isMatch = false;
            if (claimType.toLowerCase().contains('coffee') || 
                claimDescription.toLowerCase().contains(title.toLowerCase())) {
              isMatch = true;
            }
    ...
    if (mounted) {
      setState(() {
        ...
        if (points! >= 5) {
          ...
          rewardClaimed5 = true; // Not enough points for donut
          LoggingService.instance.loyalty('Not enough points for donut - ${points} < 5');
        }
  ...
  Future<void> fetchPoints({bool forceRefresh = false}) async {
    ...
    try {
      if (widget.qrToken.isNotEmpty) {
        final response = await ApiService.getUserByQrToken(widget.qrToken);
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: Column(
                        ...
                        if (!isLoading && !_isLoadingRewards && errorMsg == null && points != null) ...[
                          ...
                          // Fallback static banners if dynamic ones don't work
                          if (activeRewards.isEmpty) ...[
                            if (points! >= 5 && !rewardClaimed5)
                              FadeTransition(
                                opacity: _fadeAnimation,
                                child: _buildStaticFivePointBanner(context),
                              ),
                            if (points! >= 10 && !rewardClaimed10)
                              FadeTransition(
                                opacity: _fadeAnimation,
                                child: _buildStaticTenPointBanner(context),
                              ),
                          ],
                        ],
                        const SizedBox(height: 18),
  ...
  Widget _buildStaticFivePointBanner(BuildContext context) {
    return Card(
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            ...
            const Expanded(
              child: Text(
                'Congratulations! You earned a Free Donut!',
  ...
  Widget _buildStaticTenPointBanner(BuildContext context) {
    return Card(
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            Icon(Icons.emoji_events, color: Colors.green[800], size: 32),
            const SizedBox(width: 12),
            const Expanded(
              child: Text(
                'Amazing! You earned a Free Medium Coffee or Pastry!',
                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16),
              ),
            ),
            rewardClaimed10
  ...
  List<Widget> _buildDynamicRewardBanners(BuildContext context) {
    ...
    for (final reward in activeRewards) {
      ...
      final bannerColor = reward['bannerColor'] as String? ?? '#FFD700';
      final iconName = reward['iconName'] as String? ?? 'emoji_events';
      ...
      // Check if user has enough points and hasn't claimed this reward
      if (hasEnoughPoints && !isClaimed) {
        LoggingService.instance.loyalty('Adding banner for: $title');
        banners.add(
          FadeTransition(
            opacity: _fadeAnimation,
  ...
  void _claimReward(BuildContext context, int points) async {
    if (_isClaimingReward) return; // Prevent multiple simultaneous claims
    
    LoggingService.instance.loyalty('Claiming reward for $points points');
    ...
    String rewardText = points == 5
        ? 'You have claimed your Free Donut! Show this to the cashier.'
        : 'You have claimed your Free Medium Coffee or Pastry! Show this to the cashier.';
    String rewardType = points == 5 ? 'donut' : 'coffee';
    String rewardDesc = points == 5 ? 'Free Donut' : 'Free Medium Coffee or Pastry';
    
    try {
      // Check if still mounted before API call
      if (!mounted) return;
      ...
      LoggingService.instance.loyalty('Claim result', claimResult);
      
      if (claimResult == null) {
        throw Exception('No response from server. Please check your internet connection and try again.');
      }
      
      if (claimResult['success'] != true) {
        final errorMessage = claimResult['error'] ?? 'Unknown error occurred';
        throw Exception('Failed to claim reward: $errorMessage');
      }
      
      LoggingService.instance.loyalty('Reward claimed successfully');
      ...
      // Update points from server response first
      final newPoints = claimResult['newPoints'] ?? this.points ?? 0;
      LoggingService.instance.loyalty('New points from server: $newPoints');
      
      // Update local state based on server response - with mounted check
      if (mounted) {
        setState(() {
          // For donut claims, points don't change, so keep current points
          if (rewardType == 'donut') {
            // Don't update points for donut claims - they remain the same
            LoggingService.instance.loyalty('Donut claim - keeping points at: ${this.points}');
            // For donut rewards, don't hide banner permanently - let cycle logic handle it
            // Donut points are NOT deducted, so user can claim again in same cycle
            LoggingService.instance.loyalty('Donut claimed - points remain, banner will be re-evaluated');
          } else {
            this.points = newPoints;
          }
          if (rewardType == 'coffee') {
            rewardClaimed10 = true; // Hide coffee button after claiming (points reset to 0)
            // Coffee claim resets points to 0 - no cycle logic needed
            LoggingService.instance.loyalty('Coffee claimed - points reset to 0');
          }
        });
      }
      ...
      // Refresh reward history to update claimed status
      await fetchRewardHistory();
      
      // Re-evaluate reward button states after claiming
      // The fetchRewardHistory method already handles updating button states
      
      // For donut claims, re-evaluate the donut banner status since points weren't deducted
      if (rewardType == 'donut' && mounted) {
        _checkPreviouslyClaimedRewards();
      }
      
      // Notify parent widget about points update - with mounted check
      if (mounted && widget.onPointsUpdated != null) {
        widget.onPointsUpdated!();
      }
      
      if (mounted && widget.onPointsChanged != null) {
        widget.onPointsChanged!(this.points ?? 0);
      }
      ...
      // Show points reset message for 10-point reward
      if (rewardType == 'coffee' && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Your points have been reset to 0. Start earning for your next reward!'),
            duration: Duration(seconds: 3),
      ...
      if (mounted) {
        try {
          ...
          LoggingService.instance.warning('Error showing error message', e);
        }
      }
    } finally {
      // Reset claiming state - with mounted check
      if (mounted) {
        try {
          setState(() {
            _isClaimingReward = false;
          ...
          LoggingService.instance.warning('Error resetting claiming state', e);
        }
      }
    }
  }
