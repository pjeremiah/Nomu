...
class _BaristaScannerPageState extends State<BaristaScannerPage> with WidgetsBindingObserver {
  ...
  Future<void> _showDrinkSelectionDialog() async {
    List<Map<String, dynamic>>? selectedItems = await showDialog<List<Map<String, dynamic>>>(
      ...
      builder: (context) {
        Set<String> tempSelectedItemIds = <String>{};
            ...
            return ConstrainedBox(
              ...
              title: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  if (tempSelectedItemIds.isNotEmpty)
                    Container(
                      height: 120,
                      padding: const EdgeInsets.all(8),
                      decoration: BoxDecoration(
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(
                            'Selected Items (${tempSelectedItemIds.length}):',
                            style: const TextStyle(
                              fontWeight: FontWeight.bold,
                              fontSize: 12,
                            ),
                          ...
                          Expanded(
                            child: SingleChildScrollView(
                              child: Wrap(
                                spacing: 4,
                                runSpacing: 4,
                                children: tempSelectedItemIds.map((itemId) {
                                  ...
                                  return Chip(
                                    ...
                                    onDeleted: () {
                                      setState(() {
                                        tempSelectedItemIds.remove(itemId);
                                      });
                                    },
                                  );
                                }).toList(),
                  ...
                  children: [
                    ...
                    Expanded(
                      child: () {
                        ...
                        allItemIdsToShow.addAll(tempSelectedItemIds);
                        ...
                        return ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final isSelected = tempSelectedItemIds.contains(itemId);
                            ...
                            return Card(
                              ...
                              child: CheckboxListTile(
                                ...
                                onChanged: (bool? value) {
                                  setState(() {
                                    if (value == true) {
                                      tempSelectedItemIds.add(itemId);
                                    } else {
                                      tempSelectedItemIds.remove(itemId);
                                    }
                                  });
              ...
              actions: [
                ...
                ElevatedButton(
                  onPressed: tempSelectedItemIds.isEmpty
                      ? null
                      : () {
                          // Convert item IDs to item objects using the combined lookup
                          final selectedItems = tempSelectedItemIds
                              .map((itemId) => _getItemById(itemId))
                              .where((item) => item != null)
                              .cast<Map<String, dynamic>>()
                              .toList();
                          Navigator.of(context, rootNavigator: true).pop(selectedItems);
                        },
                  child: Text('Add ${tempSelectedItemIds.length} Item${tempSelectedItemIds.length != 1 ? 's' : ''}'),
                ),
              ],
            ),
            );
    ...
    for (Map<String, dynamic> item in selectedItems) {
      ...
      Logger.debug('Processing item: $itemName (ID: $itemId, isMenu: $isMenu, stock: $currentStock)', 'TRANSACTION');
      
      // For pure menu items (no inventory), we don't need to check stock or decrease it
      if (isMenu) {
        // Add menu item to transaction
        _addItemToTransaction(itemName);
        processedItemNames.add(itemName);
        Logger.success('Successfully processed menu item: $itemName', 'MENU');
        continue;
      }
      
      // For inventory items (including menu variations that map to inventory), try to decrease stock
      Logger.debug('Attempting to decrease stock for inventory item: $itemName (ID: $itemId)', 'INVENTORY');
      
      // Decrease stock by 1 (assuming 1 unit per sale)
      ...
      Logger.debug('Stock decrease result for $itemName: $stockResult', 'INVENTORY');
      
      if (stockResult != null && stockResult.containsKey('error')) {
        Logger.warning('Failed to decrease stock for $itemName: ${stockResult['error']} - but allowing transaction to continue', 'INVENTORY');
        // Show warning to user but don't block transaction
        if (mounted) {
          CustomToast.showWarning(
            context,
            message: 'Could not update stock for $itemName, but transaction will continue',
        ...
        if (mounted) {
          CustomToast.showSuccess(
            context,
            message: 'Stock decreased for $itemName',
              duration: const Duration(seconds: 2),
          );
        }
        Logger.success('Successfully processed $itemName and decreased stock', 'INVENTORY');
      } else {
        Logger.warning('Stock decrease failed for $itemName - but allowing transaction to continue', 'INVENTORY');
      }
  ...
  Future<void> _completeTransaction(String qrCode, String selectedDrink) async {
    ...
    if (result != null) {
      // Check if this is an error response (400 status)
      if (result.containsKey('error')) {
        ...
        // Handle error case - customer has reached maximum points
